...
def evaluate_fixture(row: dict) -> dict:
    ...
    stages = [
        Stage(
            ...
            p_correct=None if use_priors else float(stage["p_correct"]),
            detection_rate=None if use_priors else float(stage.get("detection_rate", 1.0)),
    ...
    if use_priors:
        prior_entries = []
        for item in row["prior_entries"]:
            prior_entries.append({**item, "_source_order": 0, "_source_path": "<fixture>"})

    options, best = route_options(
        stages,
        prior_entries=prior_entries,
        task_class=str(row.get("task_class", "default")),
        oracle_strength=str(row.get("oracle_strength", "mixed")),
        ...
        max_terminal_failure=float(row.get("max_terminal_failure", 1.0)),
        min_validated_correct=float(row.get("min_validated_correct", 0.0)),
